...
Menu_Item_IDs for TroopWebHost's built-in reports turned out to be **the same across every org**, not troop-specific as originally assumed (confirmed 2026-08-12) - so they're hardcoded directly in `twh/downloads.js`'s `RECIPES` object (`roster: 53747`, `requirements: 46047`, `meritBadges: 52388`, `mbRequirements: 52217`), not read from settings. `settings.js` only holds `troopName` and `subdomain` now. If you add a new TroopWebHost recipe, find its ID the same way (run the report manually, read `Menu_Item_ID=` from the URL) and hardcode it in `RECIPES` too.
...
The Merit Badge Analysis report's "Almost There" section (scouts within N requirements of finishing a badge) needs requirement-level data, which the Merit Badge History export does not carry — that file only has finished badges (`Scout, Merit Badge, Earned, Awarded`, no blank-`Earned` in-progress rows). It takes TWH's **Uncompleted Merit Badge Requirements** export instead (`RECIPES.mbRequirements` = `52217`, Advancement → Requirements Reports; confirmed 2026-09-03), wired as an *optional* auto-fetched input. That report is a **summary, not a per-requirement list** — confirmed 2026-09-03 against a live export, after an initial guess at the other shape shipped wrong numbers. One row per scout+badge, columns `Scout, Merit Badge, Started, Completed Requirements, Uncompleted Requirements, Merit Badge Counselor`, and TWH leaves finished badges out of the file entirely (no row has 0 uncompleted). So `processNearlyComplete()` in `reports/merit-badges.js` reads the `Uncompleted Requirements` count straight off the row rather than counting rows. It still accepts the *itemized* shape (`Award, Code, Uncompleted Requirement, Scout` — what the rank report returns), where the row count per scout+badge is the number outstanding; which shape arrived is decided by the columns present, and a file matching neither renders the "not the expected export" notice with the headers it saw. Badge names are matched against `OFFICIAL_BADGES` after stripping TWH's Eagle asterisk and any vintage suffix (`*Weather (2014 rqmts)`); anything unmatched (a council test-lab badge) is counted and reported in the section note rather than dropped silently. Because it is optional, a failed download for it (an account without permission for that report, say) is caught in `server.js`'s fetch loop, falls back to a fresh cached copy if there is one, and otherwise lets the rest of the report generate with the "Almost There" section showing its add-this-file prompt — only *required* inputs fail the request.
...
| Merit Badge Analysis | `merit-badges` | HTML (+ optional PDF/CSV) | Merit Badge History (+ Uncompleted Merit Badge Requirements, optional) | Merit Badge History CSV; optional Uncompleted Merit Badge Requirements CSV powers the "Almost There" section |
